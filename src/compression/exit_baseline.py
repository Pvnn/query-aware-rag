...
class ExitBaselineCompressor:
  ...
  def __init__(
    ...
    Args:
      ...
      threshold: Relevance score threshold for sentence selection (0.0-1.0)
    """
  ...
  def classify_sentence(self, query, sentence, document):
    """
    Classify if a sentence is relevant to the query.
    
    Returns:
      Tuple of (Relevance probability (0.0-1.0), Token count of the prompt)
    """
    ...
    # Extract exact token count for the prompt
  ...
  def compress_with_stats(self, query, document):
    """
    Compress document and return detailed statistics, including token usage.
    """
    sentences = self.decompose_sentences(document)
