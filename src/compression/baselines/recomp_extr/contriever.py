...
def load_retriever(model_path, pooling="average", random_init=False):
    ...
    if os.path.exists(path):
        ...
        tokenizer = contriever_utils.load_hf(transformers.AutoTokenizer, retriever_model_id)
        cfg = contriever_utils.load_hf(transformers.AutoConfig, retriever_model_id)
        ...
        if "xlm" in retriever_model_id:
            ...
            model_class = Contriever
        cfg = contriever_utils.load_hf(transformers.AutoConfig, model_path)
        tokenizer = contriever_utils.load_hf(transformers.AutoTokenizer, model_path)
        retriever = contriever_utils.load_hf(model_class, model_path)
