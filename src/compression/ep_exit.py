...
class EPExitCompressor:
    ...
    def compress(self, query: str, document: str) -> str:
        ...
        for i in range(0, len(unit_texts), self.exit.batch_size):
            ...
            batch_contexts = contexts_list[i : i + self.exit.batch_size]
            batch_units   = units[i : i + self.exit.batch_size]
            
            # Passed batch_contexts instead of a single string 'document'
            yes_probs, _  = self.exit._predict_batch(batch_queries, batch_contexts, batch_texts)
